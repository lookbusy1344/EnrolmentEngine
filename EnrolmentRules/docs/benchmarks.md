...
| `EvaluateSingle` | **13 µs** | **59 KB** | 0.31× |
...
- **Per-request evaluation is cheap.** ~13 µs and ~59 KB per student, entirely Gen0. A web endpoint
  doing one `Evaluate` per request is dominated by RulesEngine lambda invocation, not I/O or
...
- **`Evaluate` / `Explain` are the endpoints you scale.** Build one engine at startup,
