...
**Deploy target: a dedicated demo project, not a project holding anything sensitive.** `enrolment-web`
was moved to `jobtrack-demo-projects` (shared with JobTrack's own SQLite demo — see that repo's
`docs/plans/2026-08-06-cloudrun-persistent-isolation-plan.md`) so that a compromise of this public
demo cannot reach anything with real state. The script now also creates a dedicated `demo-run`
service account with no IAM roles and deploys under it (`--service-account`), rather than the
project's default compute service account, which can otherwise carry roles this service does not
need.
