...
(`DfeTransitionMatrix`) rejects rows that do not have exactly 12 fields.

A subject entirely absent from the file still falls back to empty evidence inside `FindEvidence` —
DfE probability `0`, which would otherwise pin every rating for that subject to red. That fallback
is no longer silent: `DfeTransitionMatrix.ValidateCoverage` runs at both production startup paths
(`EnrolmentEngine.Create` and the CLI's bare-path profile mode) and throws `TransitionMatrixException`
naming every catalogue subject that requires DfE evidence
(`SubjectMeta.RequiresDfeEvidence`, true by default) but has no populated band. A data-only custom
subject whose rating tiers never call `facts.DfeProbabilityAtOrAbove` may set `dfe_evidence: false`
against its `data/catalogue.yaml` entry to opt out of this guard.
