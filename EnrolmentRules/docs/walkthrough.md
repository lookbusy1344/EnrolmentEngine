...
separately from the 1–9 GCSEs. The closed type vocabulary is the compiled `QualificationType` enum
and is repeated in the qualification and catalogue schemas. For those known types,
`data/qualifications.yaml` supplies each grade's ordinal (for "at least grade X" checks) and its
**A-level-points equivalence** on the same `A*=6 … U=0` scale prediction uses:
...
constraints — so no fixpoint iteration is needed. A chain of dependent subjects (A requires B
requires C) resolves in dependency order within that phase, leaves first, so a downgrade at the
bottom propagates through every dependant above it. The catalogue enforces this at load time,
rejecting a self-prerequisite or any prerequisite cycle, so the dependency-order walk always
terminates.
...
**Routine rule configuration is entirely a YAML process** — no normal-operation rule change touches
compiled code. The first four paths below are everyday configuration. The final two —
...
**Changing qualification grades or equivalences (the scale-data path).** Edit
`data/qualifications.yaml` to add or retune a grade, its ordinal, or its A-level-points equivalence
for an existing `QualificationType`. The set of qualification types is compiled and schema-enumerated;
