	...
	EquatableArray<Prerequisite> Prerequisites)
{
	...
	/// <summary>
	///     Whether this subject's rating tiers read DfE transition-matrix evidence
	///     (<c>facts.DfeProbabilityAtOrAbove</c>). True for every shipped subject; a data-only custom subject
	///     whose tiers never call that accessor may opt out in <c>data/catalogue.yaml</c>, exempting it from
	///     the transition matrix's startup coverage guard.
	/// </summary>
	public bool RequiresDfeEvidence { get; init; } = true;
}
...
public sealed class CatalogueData
{
		...
		QualificationScale scale)
	{
		...
		// The constraint pass evaluates prerequisite chains in dependency order (ConstraintPass.PrerequisiteOrder),
		// which is only well-defined over an acyclic graph: a self-prerequisite, a two-node cycle, or a longer
		// one would let a subject's prerequisite qualify itself (directly or transitively) rather than
		// requiring an independent anchor. Every prerequisite alternative referenced here is already confirmed
		// to exist in `entries` by the loop above, so this walk cannot hit an undefined subject.
		DetectPrerequisiteCycle(entries);
	}

	private static void DetectPrerequisiteCycle(FrozenDictionary<Subject, SubjectMeta> entries)
	{
		var state = new Dictionary<Subject, CycleState>();
		var path = new List<Subject>();
		...
		{
			state[subject] = CycleState.Visiting;
			path.Add(subject);

			foreach (var required in entries[subject].Prerequisites.SelectMany(static group => group.AnyOf)) {
				if (state.GetValueOrDefault(required) == CycleState.Visiting) {
					var cycleStart = path.IndexOf(required);
					var cycle = path.Skip(cycleStart).Append(required).Select(EnumNames.NameOf);
					throw new InvalidDataException(
						$"Catalogue prerequisite graph has a cycle: {string.Join(" -> ", cycle)}.");
				}

				if (!state.ContainsKey(required)) {
					Visit(required);
				}
			}

			path.RemoveAt(path.Count - 1);
			state[subject] = CycleState.Done;
		}
	}

	private enum CycleState
	{
		// Default(CycleState) must mean "not yet seen": Dictionary.GetValueOrDefault returns this for any
		// subject not yet in `state`, and that must never be mistaken for Visiting (which would flag every
		// first-time visit to an unvisited node reached from two different parents as a false cycle).
		Unvisited,
		Visiting,
		Done,
	}
		...
		foreach (var entry in CatalogueFile.From(document).Subjects) {
			var meta = new SubjectMeta(
				...
				RestudyBar = entry.RestudyBar is CatalogueRestudyBar restudyBar
					...
					: null,
				RequiresDfeEvidence = entry.DfeEvidence ?? true,
			};
