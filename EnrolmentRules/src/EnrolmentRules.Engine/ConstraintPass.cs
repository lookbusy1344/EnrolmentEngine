...
internal static class ConstraintPass
{
	...
	///     exclusion, own-time, veto, restudy bar) read only the <em>base</em> ratings; prerequisites then
	///     read the ratings <em>after</em> those downgrades, so a dependency a sibling constraint drove to red
	///     no longer satisfies a dependent's requirement. Because none of the non-prerequisite constraints read
	///     prerequisite output, this is a fixed two-phase order — not a fixpoint iteration — and it stays
	///     acyclic. Within the prerequisite phase itself, chains are resolved in dependency order (leaves
	///     first — see <see cref="PrerequisiteOrder" />) so a downgrade at the bottom of a chain propagates
	///     through every dependant above it, not just the one directly above. Application still commutes:
	///     <see cref="Apply" /> folds the whole trail by most-severe-wins regardless of order; only this
	...
	// PrerequisiteOrder (dependencies before dependants), so a dependency's own prerequisite downgrade is
	// visible to everything above it in the chain — not just the phase-one rating. The dependent selection
	// and each adjustment's From still read the base ratings. Each unmet group emits a downgrade to its own
	// severity; most-severe-wins composes them in Apply. Driven by the Catalogue's per-subject
	// Prerequisites/EntryEquivalents tables, not hardcoded — editing those tables is how policy changes.
	...
	{
		var resolved = new Dictionary<Subject, Rating>(adjustedRatings);

		foreach (var subject in PrerequisiteOrder(catalogue)) {
			var groups = catalogue.Meta(subject).Prerequisites;
			if (groups.Count == 0 || !Qualifies(baseRatings, subject)) {
				continue;
			}

			bool Available(Subject required, PrerequisiteSatisfaction requires) =>
				IsPrerequisiteAvailable(required, requires, r => Qualifies(resolved, r), profile.ChosenALevels)
				|| HasPriorQualificationEquivalent(subject, required, profile.PriorQualifications, catalogue, scale);

			var adjustments = PrerequisiteAdjustments(subject, baseRatings[subject], groups, Available).ToArray();
			foreach (var adjustment in adjustments) {
				yield return adjustment;
			}

			if (adjustments.Length > 0) {
				var worst = adjustments.Max(static a => a.To);
				if (worst > resolved[subject]) {
					resolved[subject] = worst;
				}
			}
		}
	}

	// A dependency-first ordering of every catalogue subject: each subject's required alternatives (every
	// prerequisite group's any_of, direct dependencies only — the transitive walk is what makes the order
	// dependency-first) appear before the subject itself, so Prerequisites can resolve a chain bottom-up in
	// one pass. Well-defined because CatalogueData rejects a cyclic prerequisite graph at construction.
	private static List<Subject> PrerequisiteOrder(CatalogueData catalogue)
	{
		var visited = new HashSet<Subject>();
		var order = new List<Subject>(catalogue.Subjects.Count);
		...
		void Visit(Subject subject)
		{
			...
			foreach (var required in catalogue.Meta(subject).Prerequisites.SelectMany(static group => group.AnyOf)) {
				Visit(required);
			}
