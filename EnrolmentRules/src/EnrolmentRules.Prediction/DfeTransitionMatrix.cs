...
public sealed class DfeTransitionMatrix
{
	...
	/// <summary>
	///     Fail loud when a catalogue subject that requires DfE evidence
	///     (<see cref="SubjectMeta.RequiresDfeEvidence" />) has no populated band at all. Without this guard,
	///     a subject whose rows are entirely absent (a bad regeneration, a deleted block) silently falls back
	///     to all-zero evidence in <see cref="FindEvidence" />, which pins every rating for that subject to
	///     red rather than failing startup (see <c>docs/dfe-matrix-extraction.md</c>).
	/// </summary>
	/// <exception cref="TransitionMatrixException">
	///     One or more DfE-dependent catalogue subjects have no transition-matrix row.
	/// </exception>
	public void ValidateCoverage(CatalogueData catalogue)
	{
		...
		if (missing.Length > 0) {
			throw new TransitionMatrixException(
				$"DfE transition matrix has no rows for required subject(s): {string.Join(", ", missing)}.");
		}
	}
