...
public sealed class AdditionalSubjectsTests
{
	...
	[Fact]
	public async Task all_twelve_added_subjects_are_present_in_the_catalogue()
	{
	...
	[Fact]
	public async Task a_top_student_is_green_in_every_added_subject()
	{
	...
	[Fact]
	public async Task a_weak_student_is_red_in_every_added_subject()
	{
	...
	[Fact]
	public async Task economics_entry_requires_maths_at_strong_entry()
	{
		// English at top entry throughout, only Maths moves across the strong-entry boundary.
		var below = await RateAsync(
	...
	[Fact]
	public async Task geography_gates_on_the_humanities_average_not_a_geography_gcse()
	{
		...
		var below = await RateAsync(Uniform((int)Harness.Thresholds.HumanitiesAverageEntry - 1));
		Of(below, Subject.Geography).Should().Be(Rating.Red);

		var met = await RateAsync(Uniform((int)Harness.Thresholds.HumanitiesAverageEntry + 2));
		Of(met, Subject.Geography).Should().NotBe(Rating.Red);
	}

	[Fact]
	public async Task spanish_requires_a_modern_language_gcse_at_strong_entry()
	{
	...
	[Fact]
	public async Task economics_and_business_studies_are_a_soft_timetable_clash()
	{
