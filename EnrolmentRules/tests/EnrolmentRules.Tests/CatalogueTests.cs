...
public sealed class CatalogueTests
{
	...
	[Fact]
	public void a_direct_self_prerequisite_is_rejected()
	{
		var entries = new Dictionary<Subject, SubjectMeta> {
			[Subject.Maths] = new(50, new(0.80, -1.00), [], [], [], [new([Subject.Maths], Rating.Red)]),
		};

		var act = () => new CatalogueData(entries);

		act.Should().Throw<InvalidDataException>().WithMessage("*cycle*maths*maths*");
	}

	[Fact]
	public void a_two_node_prerequisite_cycle_is_rejected()
	{
		var entries = new Dictionary<Subject, SubjectMeta> {
			[Subject.Maths] = new(50, new(0.80, -1.00), [], [], [], [new([Subject.Physics], Rating.Red)]),
			[Subject.Physics] = new(48, new(0.90, -2.20), [], [], [], [new([Subject.Maths], Rating.Red)]),
		};

		var act = () => new CatalogueData(entries);

		act.Should().Throw<InvalidDataException>().WithMessage("*cycle*maths*physics*maths*");
	}

	[Fact]
	public void a_longer_prerequisite_cycle_is_rejected()
	{
		var entries = new Dictionary<Subject, SubjectMeta> {
			[Subject.Maths] = new(50, new(0.80, -1.00), [], [], [], [new([Subject.Physics], Rating.Red)]),
			[Subject.Physics] = new(48, new(0.90, -2.20), [], [], [], [new([Subject.Chemistry], Rating.Red)]),
			[Subject.Chemistry] = new(46, new(0.90, -2.30), [], [], [], [new([Subject.Maths], Rating.Red)]),
		};
