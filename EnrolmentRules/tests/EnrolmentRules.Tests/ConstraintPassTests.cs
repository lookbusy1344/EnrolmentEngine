...
public sealed class ConstraintPassTests
{
	...
	public void a_qualifying_prerequisite_downgrade_propagates_through_a_three_subject_chain()
	{
		...
		SubjectRating[] ratings = [
			...
			new(Subject.Chemistry, Rating.Green, "chemistry base"),
		];

		var applied = ConstraintPass.Apply(ratings, ConstraintPass.Evaluate(ratings, profile, catalogue));

		applied.Single(rating => rating.Subject == Subject.Maths).Rating.Should().Be(Rating.Red);
		applied.Single(rating => rating.Subject == Subject.Physics).Rating.Should().Be(Rating.Red);
		var chemistryFinal = applied.Single(rating => rating.Subject == Subject.Chemistry);
		chemistryFinal.Rating.Should().Be(Rating.Red);
		chemistryFinal.Reason.Should().Contain("physics");
	}

	[Fact]
	public void a_branching_prerequisite_group_is_satisfied_by_either_alternative()
	{
		...
		adjustments.Should().NotContain(adjustment => adjustment.Subject == Subject.Economics);
	}
}
