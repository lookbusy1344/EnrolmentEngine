...
public sealed class ChooseRemoveSubjectTests : IClassFixture<WebAppFactory>
{
	...
	public async Task An_amber_choice_is_flagged_borderline_in_the_basket_and_a_green_one_is_not()
	{
		...
		var html = await afterChoose.Content.ReadAsStringAsync();

		// The amber choice keeps its amber pill and says why it is not simply accepted; the green one is unchanged.
		html.Should().Contain("list-inline-item badge text-bg-warning rounded-pill\">Music");
		html.Should().Contain("list-inline-item badge text-bg-primary rounded-pill\">History");
		html.Should().Contain("borderline-notice");
		html.Should().Contain("additional authorisation");
	}
