...
public sealed partial class SaveFactsPostTests : IClassFixture<WebAppFactory>
{
	...
	public async Task Posting_a_grade_off_the_scale_normalises_it_before_it_reaches_the_stored_snapshot(string posted, string expected)
	{
		...
		var checkedGrade = CheckedGcseGradeRow0Regex().Match(html);
		checkedGrade.Success.Should().BeTrue("exactly one grade radio for row 0 should be checked");
		checkedGrade.Groups[1].Value.Should().Be(expected);
	}
	...
	[GeneratedRegex("id=\"Gcses_0__Grade_(\\d)\"[^>]*checked=\"checked\"")]
	private static partial Regex CheckedGcseGradeRow0Regex();
}
