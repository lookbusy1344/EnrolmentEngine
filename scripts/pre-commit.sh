...
# The git repository root holds the EnrolmentRules .NET solution in a subdirectory,
# so every dotnet invocation must target the solution explicitly.
readonly SOLUTION="EnrolmentRules/EnrolmentRules.slnx"
...
main() {
  ...
  gtimeout "$TEST_TIMEOUT_SECONDS" dotnet test "$SOLUTION"
}
